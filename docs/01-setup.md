...
|---|---|---|
| `CHAT_PROVIDER` | Which LLM backs the chat agent (Step 8): `anthropic`, `openai`, `xai`, or `perplexity`. | `anthropic` |
...
| `PERPLEXITY_API_KEY` | Required if `CHAT_PROVIDER=perplexity`. Tool-calling support isn't confirmed reliable for this provider -- see Step 8. | unset |
| `CHAT_MODEL` | Which model the selected provider uses. Default depends on `CHAT_PROVIDER`: `claude-opus-4-8` (anthropic), `gpt-4o` (openai), `grok-4` (xai), `sonar-pro` (perplexity). | see left |
...
89 tests should pass. If they don't, something in your environment
